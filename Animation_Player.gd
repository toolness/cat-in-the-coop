...
func _ready():
	...
	var result = connect("animation_finished", self, "animation_ended")
	if result != OK:
...
func set_animation(animation_name):
	if animation_name == current_state:
		print("WARNING: animation is already ", animation_name)
		return true
	...
	if has_animation(animation_name):
		if current_state != null:
			...
			if animation_name in possible_animations:
				...
				print("WARNING: cannot change to ", animation_name, " from ", current_state)
				return false
		else:
			current_state = animation_name
			play(animation_name, -1, animation_speeds[animation_name])
			return true
	print("WARNING: animation name not found: ", animation_name)
	return false


func animation_ended(_animation_name):
	# UNARMED transitions
...
func animation_callback():
	if callback_function == null:
		print("WARNING: no callback function for the animation to call!")
	else:
